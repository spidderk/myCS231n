...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    hidden, hcache = affine_relu_forward(X, W1, b1)
    scores, scache = affine_relu_forward(hidden, W2, b2)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dhidden, dW2, db2 = affine_relu_backward(dscores,scache)
    dx, dW1, db1 = affine_relu_backward(dhidden,hcache)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers):
        ...
        h = hidden['h'+str(i)]
        dcache = 0
        if (i == self.num_layers - 1): # output layer, no ReLU
            h, cache = affine_forward(h, W, b)
            #hidden['h'+str(i+1)] = h
            #hidden['cache'+str(i+1)] = cache
        else:
            if self.use_batchnorm:
                h, cache = affine_bn_relu_forward(h, W, b, 
    ...
    for i in xrange(self.num_layers,0,-1):
        if (i == self.num_layers): # output layer, no BN or ReLU
            dx, dw, db = affine_backward(dx,hidden['cache'+str(i)])
        else:
            if self.use_dropout:
                dx = dropout_backward(dx, hidden['dcache'+str(i)])
            if self.use_batchnorm:
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx,hidden['cache'+str(i)])
                grads['gamma'+str(i)] = dgamma
                grads['beta'+str(i)] = dbeta
            else:
                dx, dw, db = affine_relu_backward(dx,hidden['cache'+str(i)])
        W = self.params['W'+str(i)]
        dw += reg * W
        grads['W'+str(i)] = dw
        grads['b'+str(i)] = db.reshape((-1,))
