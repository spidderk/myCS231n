...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0, dp=0.0):
    """
    ...
    if (dp > 0):
        #mask = 1/(1-dp)*np.random.binomial(1,dp,size = W2.shape[0])
        mask = (np.random.rand(*hidden_layer.shape) < dp)/dp
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      loss, grads = self.loss(X_batch, y=y_batch, reg=reg, dp=dp)
